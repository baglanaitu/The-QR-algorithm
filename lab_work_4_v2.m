...
% Get only the diagonal values and in the right order
D = diag(A_k)';
D = D(end:-1:1);
...
% Set up A with the smallest value of the results of ex1
A = A - eye(n)*A_k(n);
...
function [Q,R] = QR(M)
    ...
    [n,~] = size(M);
    Q = M;
    % Create a array of matrices, for each intermediary matrix of Q
    Q_s = zeros(n,n,n);
    for k=1:n
